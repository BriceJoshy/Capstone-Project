...
const MenShirt = () => {
  ...
  const prevImage = (shirtId) => {
    setCurrentImageIndex(prev => {
      const currentIndex = prev[shirtId] || 0;
      const prevIndex = (currentIndex - 1 + shirts.find(shirt => shirt.productId === shirtId).productImageUrl.length) % shirts.find(shirt => shirt.productId === shirtId).productImageUrl.length;
  ...
  return (
    <div className="container mx-auto p-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-1">
      {shirts.map(shirt => (
        <div key={shirt.productId} className="group bg-white shadow-lg  p-2 transition-all duration-300 hover:shadow-2xl">
          <div className="relative overflow-hidden ">
            ...
            {shirt.productImageUrl.length > 1 && (
              <>
              <button
                onClick={() => prevImage(shirt.productId)}
              ...
              <button
                onClick={() => nextImage(shirt.productId)}
